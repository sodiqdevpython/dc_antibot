...
namespace dc_antibot.AntiBot.Modules.Keylogger
{
    public class KeyloggerModule : ModuleBase
    {
        public override string Name { get { return "Keylogger"; } }

        private const int MaxWatched = 512;

        private static readonly HashSet<string> KeyloggerApis = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            "setwindowshookexw",
            "setwindowshookexa",
            "getasynckeystate",
        ...
        private static bool IsKeyloggerApi(string api)
        {
            if (string.IsNullOrEmpty(api)) return false;
            return KeyloggerApis.Contains(api);
        }
        ...
        private void OnApiCalled(object sender, ApiEvent e)
        {
            ...
            string reason = "Keylogger activity via " + api;
